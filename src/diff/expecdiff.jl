...
function _qterm_expec_util(m::QubitsTerm, state::MPS; trunc::TruncationScheme=DefaultMPSTruncation, alg::AbstractMPSAdd=IterativeAdd(D=trunc.D))
...
function _qop_expec_util(m::QubitsOperator, state::MPS; trunc::TruncationScheme=DefaultMPSTruncation, alg::AbstractMPOMPSMult=IterativeMult(D=trunc.D))
	return expectation(m, state), z -> begin
		...
		if _is_herm
			...
			b[1] .*= z
			mpsout, err = mpsadd([a, b], IterativeAdd(D=alg.D, maxiter=alg.maxiter, tol=alg.tol, verbosity=alg.verbosity))
		end
